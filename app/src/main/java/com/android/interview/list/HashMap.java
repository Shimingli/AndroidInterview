...
//     * to incorporate impact of the highest bits that would otherwise table[index]
...
//
//
//    // todo  存储（位桶）的数组</k,v> 因此使用了 transient 修饰，可以防止被自动序列化。   java语言的关键字，变量修饰符，如果用transient声明一个实例变量，当对象存储时，它的值不需要维持。换句话来说就是，用transient关键字标记的成员变量不参与序列化过程。
...
//        size = 0;
//    }
//    //三个方法都是为了继承HashMap的LinkedHashMap类服务的。  LinkedHashMap 是 HashMap 的一个子类，它保留插入的顺序，如果需要输出的顺序和输入时的相同，那么就选用 LinkedHashMap。
...
//     * linked node.
//     */
//    // todo  红黑树：是一种自平衡二叉查找树，是计算机科学中用到的一种数据结构，典型的用途就是实现关联数组
//    static final class TreeNode<K,V> extends LinkedHashMap.LinkedHashMapEntry<K,V> {
//        TreeNode<K,V> parent; //  父节点// red-black tree links
//        TreeNode<K,V> left;//左子树
//        TreeNode<K,V> right;//右子树
//        TreeNode<K,V> prev;   //删除时需要断开链接 // needed to unlink next upon deletion
//        boolean red;// 颜色属性
