...
public class FrameLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        //孩子的总数 同级孩子的总数，一般为1，为啥呢，就是因为我们常规写个布局Activity
        //你肯定是 写一个Layout 把他们全部包裹在一起的啊  对吧
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (mMeasureAllChildren || child.getVisibility() != GONE) {
                // TODO: 2018/5/31  会去调用孩子。chlde.measure方法  第一次会走 孩子的onMeasure
                measureChildWithMargins(child, widthMeasureSpec, 0, heightMeasureSpec, 0);
                final LayoutParams lp = (LayoutParams) child.getLayoutParams();
                //通过断点去调试，可以知道如果是match_parent 的就是屏幕的宽高
        ...
        //注意这个地方是从新的赋值，不是重新 new 个对象出来
        count = mMatchParentChildren.size();
        // TODO: 2018/5/31 联系到你自己写activity ，count会被重置为0的  那么这里也进不来
        if (count > 1) {
            for (int i = 0; i < count; i++) {
                ...
                // TODO: 2018/5/31  这里也没戏  断点源码建议使用模拟器，真机一般都是修改过的，与SDK代码不一致。
                /*
                以FrameLayout为例，确实是在onMeasure方法中对child进行了2次测量，但这是有条件限制的，需要FrameLayout的layout_width/height属性不能为match_parent或具体的值，且child的layout属性必须为match_parent，具有特殊性，实际上即使不满足以上条件依旧会进行2次测量
                 */
