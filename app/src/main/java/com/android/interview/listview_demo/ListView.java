...
public class ListView extends AbsListView {
    ...
    // TODO: 2018/6/20  ViewGroup 是如何去layout的

    /**
     * onLayout()方法中并没有做什么复杂的逻辑操作，
     * 主要就是一个判断，如果ListView的大小或者位置发生了变化，那么changed变量就会变成true，
     * 此时会要求所有的子布局都强制进行重绘。除此之外倒没有什么难理解的地方了，不过我们注意到，
     * 在第16行调用了layoutChildren()这个方法，从方法名上我们就可以猜出这个方法是用来进行子元素布局的，
     * 不过进入到这个方法当中你会发现这是个空方法，没有一行代码。这当然是可以理解的了，
     * 因为子元素的布局应该是由具体的实现类来负责完成的，而不是由父类完成。
     * 那么进入ListView的layoutChildren()方法
     */
    @Override
